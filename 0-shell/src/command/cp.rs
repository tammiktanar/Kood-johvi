...
pub fn handle_command(ctx: &mut CommandCtx) -> CmdReturn {
    ...
    if paths.is_empty()  {
        writeln!(ctx.stderr, "cp: missing file operand")?;
        writeln!(ctx.stderr, "Try 'cp --help' for more information.")?;
        ...
        if paths.len() < 2 && output_path.is_empty() {
            writeln!(ctx.stderr, "cp: missing destination file operand after {}", paths.last().unwrap())?;
            writeln!(ctx.stderr, "Try 'cp --help' for more information.")?;
            ctx.stderr.flush()?;
        ...
        if paths.len() == 1 {
            ...
            if !temp_path.is_dir() && !temp_path.is_file() {
                ...
                if !path.is_dir() && !path.is_file() && !path.exists() {
                    writeln!(ctx.stderr, "cp: cannot stat '{}': No such file or directory", path.display())?;
                    return Ok(1)
                } else {
                    fs::copy(paths.last().unwrap(), &output_path)?;
                    return Ok(0)
                }
        ...
        if !temp_path.is_dir() { // If output path is not a directory
            writeln!(ctx.stderr, "cp: failed to access '{}': No such file or directory", &output_path)?;
        ...
        for path_string in paths.iter() {
            ...
            if !path.is_dir() && !path.is_file() && !path.exists() {
                writeln!(ctx.stderr, "cp: cannot stat '{}': No such file or directory", path.to_str().unwrap())?;
            } else if &output_path == path_string {
                if path.is_dir() {
                    writeln!(ctx.stderr, "cp: cannot move '{}' to a subdirectory of itself, '{}'", path.to_str().unwrap(), &output_path)?;
                } else {
                    writeln!(ctx.stderr, "cp: cannot stat '{}': No such file or directory", path.to_str().unwrap())?;
                }
            } else {
